...
(define (domain VGDLGame)  
	...
	(:types
		...
		crab - enemy
		butterfly - enemy
		Flicker Door Resource RandomNPC movingStype Immovable ShootAvatar Missile - Object
	)
	...
	(:action ENEMY_DIRT_STEPBACK
		:parameters (?x - enemy ?y - dirt ?c - cell ?c_last - cell)
		:precondition (and
						(turn-interactions)
						(not (= ?x ?y))
						(at ?c ?x)
	...
	(:action ENEMY_DIAMOND_STEPBACK
		:parameters (?x - enemy ?y - diamond ?c - cell ?c_last - cell)
		:precondition (and
						(turn-interactions)
						(not (= ?x ?y))
						(at ?c ?x)
